...
-- Version 1.3 30/12/2014 - Ajout du nom de l'adresse : AddressName char(100)
--                          Ajout état d'un nouvel utilisateur : Validated enum('Y','N') default 'N',
--                          Création d'une table de validation des utilisateurs
-- Version 1.4 03/01/2015 - Ajout table des messages
--             04/01/2015 - Ajout pseudo sur les utilisateurs
--                          Ajout sens d'emission d'un message
-- Version 1.5 21/01/2015 - Ajout champ région sur les adresses
-- Version 1.6 28/02/2015 - Refonte table Message
-- Version 1.7 29/03/2015 - Refonte logs
...
create table if not exists logs (
  ...
  index idx_logs_Priority (Priority)
  );
...
-- Création de la base de contrôle des nouveaux utilisateurs
-- La date est au format Unix Timestamp
-- Au bout de 4 heures l'utilisateur est annulé
create table if not exists ControlNewUsers (
  Id char(40) primary key not null,
  UserId bigint,
  DateRequest integer
);

-- Création d'une table de message entre utilisateurs
-- Note : MessageBox IN  demandeur vers dépositaire
--                   OUT dépositaire vers demandeur
create table if not exists Messages (
  ...
  CreateDate DateTime not null,
  Message text,
  MessageSee enum('Y','N') default 'N',
  MessageBox varchar(80),
  index idx_message_user1 (UserFromId),
  index idx_message_user2 (UserToId),
  constraint foreign key fk_message_user1(UserFromId) references MainUser(Id),
  constraint foreign key fk_message_user2(UserToId) references MainUser(Id),
  constraint foreign key fk_message_piece(PieceId) references Pieces(Id)
);
